...
namespace NuGet.V3Repository
{
    public abstract class CatalogWriterBase : IDisposable
    {
        ...
        public Task<IEnumerable<string>> Commit(IGraph commitMetadata, CancellationToken cancellationToken)
        {
            return Commit(DateTime.UtcNow, commitMetadata, cancellationToken);
        }

        public virtual async Task<IEnumerable<string>> Commit(DateTime commitTimeStamp, IGraph commitMetadata, CancellationToken cancellationToken)
        {
            if (!_open)
            {
                throw new ObjectDisposedException(GetType().Name);
